 ...
 */
$.fn.rletSearchPopup = function(popUrl, rletSearchUrl, callback) {

	var popupId = "rletPopup";
	var popupVo = {};
	var $layer = null;
	var	rletVo = null;
	...
	function init() {
		...
		// 팝업 레이어 불러오기
//	    appendLayer("/personal/refinRletSearchPopup.do");
	    appendLayer(popUrl);
	};
	...
	function getRletRdInfoAjax(addr, trlCd) {
		$.ajax({
			...
			data : {
                ...
                'addrDstc' : 3, 					// 주소구분 - 1 : 도로명주소 2 : 지번   3 : 간편검색
                'inqDstc' : 1, 						// 조회구분 - 1 : 아파트, 2 : 레이다
                'rletDstc' : 5,					    // 부동산구분
                'trlCd' : trlCd, 					// 시도
                'addr'  : addr  					// 전체 주소
                },
	...
	function getPropertyPriceAjax(result) {
		$.ajax({
			...
			data : {
                ...
                'rletDstcCd' :  result.rletDstcCd, //부동산구분코드
                'rletLctn' : result.rletLctn, //부동산소재지
                'rletStrtNmLctn' : result.rletStrtNmLctn, //부동산도로명소재지
                'ownr' : result.ownr, //소유자
                'sts' : result.sts, //상태
                },
			success : function(data) {
				if(data.RSPCD == "0000") {
...
/**
*
* 부동산 정보 추가입력 02 팝업 (for jQuery Selector Methods)
*
* @param callback 함수
*
* 조회 팝업을 띄워야할 버튼을 셀렉터로 선택 ex : $(셀렉터).rletSearch02Popup(function(result){});
*
* result 리턴값 : { "dwlFrmnDvcd" : 주거형태구분코드, "rlRsdnYn" : 실거주 여부, "hsngFrmnDvcd" : 주거종류, "rletAmt" : 부동산금액 }
*/
$.fn.rletSearch02Popup = function(popUrl, rletSearchUrl, callback) {

	var popupId = "rlet02Popup";
	var popupVo = {};
	var $layer = null;
	var	rletVo = null;
	...
	function appendLayer(url) {
		...
		$.get(url, function(result) {
			...
			$("#rletInfo").click(function(e) {
				...
				if(isTestMode == "true") {
					if (confirm("[테스트모드] 주소조회 실행여부 선택\n\n[확인]:실행  [취소]:패스")) { //확인눌렀을때
						$("#rletInfo").srchAddrPopupRlet(10, function(result){
							var wrstZpcd = result.wrstZpcd;//우편번호
							var wrstAddr = result.wrstAddr1 + "," + result.wrstDtad;
							var wrstAddr1 = result.wrstAddr1;//도로명주소
							var wrstAddr2 = result.wrstAddr2;//지번주소
							...
							$("#rletLctnZpcd").val(result.wrstZpcd);  //우편번호
							$("#rletLctnAddr").val(result.wrstAddr1); //주소
							$("#rletLctnDtad").val(result.wrstDtad);  //상세주소

							var rletInfo = $layer.find("#rletLctnAddr").val() + "\n" + $layer.find("#rletLctnDtad").val();
							$("#rletInfo").css("color", "#222426");
							$("#rletInfo").text(rletInfo); // 부동산 주소
							...
							$("#rletLctnZpcd").val(wrstZpcd);  //우편번호
							$("#rletLctnAddr").val(wrstAddr1); //주소
							$("#rletLctnDtad").val(wrstDtad);  //상세주소
			...
			// ***값 매핑***
			// 주택명의
			initCustomSelectPopupIn($layer.find("#dwlFrmnDvcd"), $layer.find("[name='dwlFrmnDvcd']").val());
			// 실거주 여부
			initCustomSelectPopupIn($layer.find("#rlRsdnYn"), $layer.find("[name='rlRsdnYn']").val());
			// 주거종류
			initCustomSelectPopupIn($layer.find("#hsngFrmnDvcd"), $layer.find("[name='hsngFrmnDvcd']").val());
			// 부동산 주소
			var rletInfo = "검색하세요";
			if($layer.find("#rletLctnZpcd").val() != ""
				&& $layer.find("#rletLctnAddr").val() != ""
				&& $layer.find("#rletLctnDtad").val() != "") {

				rletInfo = $layer.find("#rletLctnAddr").val() + "\n" + $layer.find("#rletLctnDtad").val();

			} else {
				$("#rletInfo").css("color", "#717171");
			}
			$("#rletInfo").text(rletInfo);
	...
	function initCustomSelectPopupIn(obj, dataCode) {
		if(dataCode == "" || (dataCode == "undefined") || (dataCode == null)) {
			return;
		}

		var list = getSelectList(obj.attr("id"));
		var dataIndex = list.findIndex(function(item) {
			return item.val == dataCode;
		});

		obj.data("index", dataIndex);
		obj.next("input").val(dataCode);
		obj.next("input").data("index", dataIndex);
		obj.val(list[dataIndex].text);

		//hidden 값 이벤트 트리거 설정
		setChangeEvent(obj.attr("id"), dataCode);
	}
	...
	function forwardSendForm() {
		...
		const datas = {
			...
			'dwlFrmnDvcd' : $layer.find("[name='dwlFrmnDvcd']").val(),
			'hsngFrmnDvcd' : $layer.find("[name='hsngFrmnDvcd']").val(),
			'rletLctnZpcd' : $layer.find("[name='rletLctnZpcd']").val(),
			'rletLctnAddr' : $layer.find("[name='rletLctnAddr']").val(),
			'rletLctnDtad' : $layer.find("[name='rletLctnDtad']").val(),
		}
	...
	function validate() {
		...
		var rletLctnZpcd = $layer.find("[name='rletLctnZpcd']"); // 우편번호
		var rletLctnAddr = $layer.find("[name='rletLctnAddr']"); // 주소
		var rletLctnDtad = $layer.find("[name='rletLctnDtad']"); // 상세주소
		if (rletLctnZpcd.val() == "" || rletLctnAddr.val() == "" || rletLctnDtad.val() == "") {
			alert("부동산 주소를 검색해 주세요.");
			$layer.find("#rletInfo").focus();
			return false;
		}
