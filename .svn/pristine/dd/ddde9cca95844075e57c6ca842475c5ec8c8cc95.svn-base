...
const AUTHTYPE_OF_KAKAO = "PLA311";
...
function kakaoResultClick() {
	var elcSignFlag = (typeof AUTH_POPUP_TYPE !== "undefined") && AUTH_POPUP_TYPE == "N" ? true : false;
	pollKakaoResult($("#tx_id").val(), elcSignFlag);
}

function pollKakaoResult(tx_id, elcSignFlag) {

	var formData = new FormData();
    formData.append("tx_id", tx_id);
    formData.append("elcSignFlag", elcSignFlag);
	...
	$.ajax({
        ...
        global: false,
        data : formData,
        processData : false, // 데이터 직렬화하지 않도록
        contentType : false, // FormData를 사용할 때는  multipart/form-data 타입이기 떄문에 false
        noUseIncSecureKey : true,
        success:function(data){
        	...
        	if (data.result == "0000") {
        		...
        		// 기간 만료
        			...
        			alert("카카오 인증이 완료된 후에 인증완료 버튼을 눌러주세요.");
        		}
        		else {
	...
	$.ajax({
        ...
        success:function(data){
        	...
        	if (data.result == "0000") {
        		showKakaoLoading();
        		$("#tx_id").val(data.tx_id);
        		location.href="kakaotalk://me/cert/sign?tx_id="+data.tx_id
        	} else if(data.result == "8888"){
        		alert("보안상의 세션제한 시간이 초과 하였습니다. 처음부터 다시 진행하여 주십시요.");
                location.reload();
        	} else {
