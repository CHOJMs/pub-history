 ...
 */
	...
	function srchRegisterAjax() {
		...
		$('#dtl1Grid tbody').html('<tr><td colspan="7">조회결과가 없습니다.</td></tr>');

		$('#dtl2Grid tbody').empty();
		$('#dtl2Grid tbody').html('<tr><td colspan="11">조회결과가 없습니다.</td></tr>');

		$('#dtl3Grid tbody').empty();
		$('#dtl3Grid tbody').html('<tr><td colspan="11">조회결과가 없습니다.</td></tr>');

		$('#dtl4Grid tbody').empty();
		$('#dtl4Grid tbody').html('<tr><td colspan="5">조회결과가 없습니다.</td></tr>');
		$.ajax({
	  			...
	  			if(data.RSPCD == "0000") {
					...
					$('#rlrgstResult2 tbody').empty();
		       		...
		       		if((list_bsicGrid == null ||  list_bsicGrid.length == 0 )) {
		       			$('#rlrgstResult2 tbody').html('<tr><td colspan="10">조회결과가 없습니다.</td></tr>');
					}
					...
					$('#rletPriceResult2 tbody').empty();
		       		...
		       		if((list_bsicGrid2 == null ||  list_bsicGrid2.length == 0 )) {
		       			$('#rletPriceResult2 tbody').html('<tr><td colspan="9">조회결과가 없습니다.</td></tr>');
					}
					if(list_bsicGrid2) {
			          	var template_bsicGrid2 = '<tr class="link">';
			          	template_bsicGrid2	+= '<td>';
			          	if(list_bsicGrid2.length == 1){
				          	template_bsicGrid2	+= '	<input type="radio" id="rRletPriceSrchInfo{0}" name="rRletPriceSrchInfo" title="선택" checked="checked">';
			          	} else {
				          	template_bsicGrid2	+= '	<input type="radio" id="rRletPriceSrchInfo{0}" name="rRletPriceSrchInfo" title="선택">';
			          	}
			          	template_bsicGrid2	+= '</td>';
	...
	srchLandPriceLayerPopVo.fnInitSetting = function() {
		...
		$pop.find('.content_tab:first').show();
		$pop.find('.list_tab:first').show();
		$pop.find('.list_tab2:first').show();
		$pop.find('.list_tab3:first').show();

		if($pop.find('ul.tab1 li a.active').attr("rel")== "rlrgst_content") {
		...
		$pop.find('#btnRegResult').click(function(e) {
			...
			// 부동산등기 활성화
			if($pop.find("#rlrgst_tab").hasClass("active")) {
...
//				}
			...
			// 부동산시세 활성화
			else if($pop.find("#rlet_tab").hasClass("active")) {
			...
			// 레이다시세 활성화
			else if($pop.find("#rd_tab").hasClass("active")) {
				return false;
			}
			...
			$.ajax({
				...
				complete : function() {
					$pop.find("p.closeBtn a").click();
				}
			});
		});

		// 부동산/레이다 조회 버튼 이벤트 설정
		$pop.find('#btnPriceSrch').click(function(e) {
			e.preventDefault();
		  			...
		  			if(data.RSPCD == "0000") {
						...
						$pop.find('#dtl4Grid tbody').empty();
			       		...
			       		if((list_dtl1Grid == null ||  list_dtl1Grid.length == 0 )) {
			       			$pop.find('#dtl1Grid tbody').html('<tr><td colspan="7">조회결과가 없습니다.</td></tr>');
						}
			       		if((list_dtl2Grid == null ||  list_dtl2Grid.length == 0 )) {
			       			$pop.find('#dtl2Grid tbody').html('<tr><td colspan="11">조회결과가 없습니다.</td></tr>');
						}
			       		if((list_dtl3Grid == null ||  list_dtl3Grid.length == 0 )) {
			       			$pop.find('#dtl3Grid tbody').html('<tr><td colspan="11">조회결과가 없습니다.</td></tr>');
						}
			       		if((list_dtl4Grid == null ||  list_dtl4Grid.length == 0 )) {
			       			$pop.find('#dtl4Grid tbody').html('<tr><td colspan="5">조회결과가 없습니다.</td></tr>');
						}
					...
					if(data.RSPCD == "0000") {
						...
						$pop.find('#rlrgstResult1 tbody').empty();
			       		...
			       		if((list_rlrgst == null ||  list_rlrgst.length == 0 )) {
			       			$pop.find('#rlrgstResult1 tbody').html('<tr><td colspan="7">조회결과가 없습니다.</td></tr>');
							return;
						}
						...
						if(inqDstc == "1") // 부동산시세 조회
						{
							...
							$pop.find('#rletPriceResult tbody').empty();
				       		...
				       		if((list == null ||  list.length == 0 )) {
				       			$pop.find('#rletPriceResult tbody').html('<tr><td colspan="10">조회결과가 없습니다.</td></tr>');
								return;
							}
						...
						else if(inqDstc == "2") //레이다시세 조회
						{
							...
							$pop.find('#rdPriceResult tbody').empty();
				       		...
				       		if((list == null ||  list.length == 0 )) {
				       			$pop.find('#rdPriceResult tbody').html('<tr><td colspan="5">조회결과가 없습니다.</td></tr>');
								return;
							}
				...
				success : function(data) {
					...
					$pop.find('#mltpResult tbody').empty();
		       		...
		       		if((bsicList == null || bsicList.length == 0 )) {
		       			$pop.find('#rletBsicResult tbody').html('<tr><td colspan="3">조회결과가 없습니다.</td></tr>');
					}

					if((commList == null || commList.length == 0 )) {
		       			$pop.find('#commResult tbody').html('<tr><td colspan="9">조회결과가 없습니다.</td></tr>');
					}

					if((kbList == null || kbList.length == 0 )) {
		       			$pop.find('#kbResult tbody').html('<tr><td colspan="14">조회결과가 없습니다.</td></tr>');
					}

					if((aptList == null || aptList.length == 0 )) {
		       			$pop.find('#aptResult tbody').html('<tr><td colspan="9">조회결과가 없습니다.</td></tr>');
					}

					if((mltpList == null || mltpList.length == 0 )) {
		       			$pop.find('#mltpResult tbody').html('<tr><td colspan="9">조회결과가 없습니다.</td></tr>');
					}
		...
		function getRadarPriceAjax() {
			$.ajax({
				...
				success : function(data) {
					...
					if((bsicList == null || bsicList.length == 0 )) {
		       			$pop.find('#rdBsicResult tbody').html('<tr><td colspan="16">조회결과가 없습니다.</td></tr>');
					}

					if((trList == null || trList.length == 0 )) {
		       			$pop.find('#trResult tbody').html('<tr><td colspan="7">조회결과가 없습니다.</td></tr>');
					}

					if((rentList == null || rentList.length == 0 )) {
		       			$pop.find('#rentResult tbody').html('<tr><td colspan="6">조회결과가 없습니다.</td></tr>');
					}
	...
	$(document).ready(function(){
		srchLandPriceLayerPopVo.fnInitSetting(); // 팝업 이벤트 초기 설정 함수 호출
	});