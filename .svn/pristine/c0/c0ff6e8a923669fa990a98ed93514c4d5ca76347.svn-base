...
$.fn.filerLayer = function(callback, opt) {
	...
	return this.each(function(i, ele) {
		...
		$(ele).click(function(e) {
			$eventSrc = $(ele);
			appendLayer('/common/filerPopup.do');
			return false;
		});
	});
...
//		        changeInput: '<div class="num"><p class="jFiler-input-caption">업로드할 파일을 추가해주세요.</p><div class="jFiler-file-box"></div></div><div class="btn_file_box"><button type="button" class="btn">파일 추가</button></div>',
			    ...
			    thumbnails: {
		            ...
		            // {{fi-name}} {{fi-size2}}
		            item: '<li class="jFiler-file-item" title="{{fi-name}}">{{fi-name}} <a href="#" class="btn-clear" role="button" title="파일삭제"></a></li>',
		            item2: null,
		            _selectors: {
...
//		            feedback: "업로드할 파일을 추가해주세요.",
		            feedback: function(t) {
		            	...
		            	$layer.find(".jFiler-input-caption").html("‘등록하기’ 버튼을 선택 후<br> 첨부하실 파일을 추가해주세요.");
