import UseBOCR from '/static/web/js/ocr.js?v=20240219001';
  ...
  if (customUI && useTextMsg) {
    ...
    switch (inProgress) {
      ...
      case ocr.IN_PROGRESS.READY:
        textMsg = `${cardTypeString}을 영역 안에 꽉 차도록 맞추면<br/>자동으로 촬영됩니다.`;
        break;
      ...
      case ocr.IN_PROGRESS.CARD_DETECT_FAILED:
        textMsg = `${cardTypeString}이 인식되지 않습니다. <br/>영역 안에 ${cardTypeString}을 꽉 차도록 맞춰주세요.`;
        break;
      ...
      case ocr.IN_PROGRESS.OCR_SUCCESS:
        textMsg = `${cardTypeString} 인식이 완료되었습니다.`;
        break;
