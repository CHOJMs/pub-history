$(function(){
    ...
    if($(window).scrollTop() == 0){
        $('#header, .main-line, .sub-line, header.bg-none').removeClass('fixed');
    }else{
        $('#header, .main-line, .sub-line, header.bg-none').addClass('fixed');
    }
...
//            $(this).next('ul').slideToggle();
	...
	$('.all-menu-m > button').click(function(){
		...
		$('.mo-menu > ul > li').children('a').attr('aria-expanded','false');
		$('.mo-menu > ul > li.on').children('a').attr('aria-expanded','true');
	});
	...
	$(window).on('load resize', function() {
        $('.center').slick('resize');
		$('.all-banner-wrap .slick-slide').removeAttr('tabindex');
    });

	//mobile 전체메뉴의 메뉴목록 선택
	$('.mo-menu > ul > li > a').click(function(){
		$('.mo-menu > ul > li').removeClass('on').find('a').attr('aria-expanded','false');
		$(this).parent('li').addClass('on').find('a').attr('aria-expanded','true');
	});
	
	// all-menu-left에 포커스 가능하도록 설정
	$('.all-menu-left').attr('tabindex', '0');

	//포커스 이동 공통
	function handleFocusOut($area, $nextFocusTarget) {
		$area.on('focusout', function (e) {
			const $next = $(e.relatedTarget);
			if (!$area.has($next).length) {
				e.preventDefault();
	...
	// 모바일 전체 메뉴 포커스 이동 설정
	handleFocusOut($('.all-menu-left'), $('.all-menu-cont-m .slick-current .m-bn-cont')); //좌측상단 -> 하단 배너로
	handleFocusOut($('.mo-menu'), $('.all-menu-right li:first-child a')); //메뉴목록 -> 우측 상단 버튼로
	handleFocusOut($('.all-menu-right'), $('.all-menu-left')); //우측상단 -> 좌측상단으로

	//헤더 로고 페이지 전환 링크 (mobile)
	function initMobileSubMenu() {
		const $menu = $('.digi-sub-menu-m');
		const $menuParent = $menu.parent();
		const $menuNext = $menu.next();
		...
		function closeMenu() {
			...
			$('.h-dim').fadeOut(200, function(){
				$(this).remove();
			});
			// transition 끝난 뒤에 DOM 원위치 복귀
			$menu.one('transitionend', function() {
				$('body').css('overflow-y','auto');
				if ($menuNext.length) {
					$menuNext.before($menu);
				} else {
					$menuParent.append($menu);
				}
			});
		}
		$('.h1-link-box-m .link').on('click', function(e) {
			e.preventDefault();
			openMenu();
		});
		$('.h1-link-box-m .btn-close').on('click', function(e) {
			e.preventDefault();
			closeMenu();
		});
	}
	initMobileSubMenu();
	$('.digi-sub-menu-m a').click(function(){
		$('.digi-sub-menu-m').find('li').removeClass('on');
