 ...
 */
	...
	srchLandPriceLayerPopVo.fnOpenPop = function(){
		...
		// 만기연장이고 여신번호가 존재하지 않을 때,
		if($("#indvMortPropKncd").val() == 'GL1307' && ($('#lonCnslNo').val() == "" || $('#lonCnslNo').val() == null)){
			alert('만기연장의 경우 "수정" 버튼 선택하여 수정 후 부동산 조회가 가능합니다.');
			return false;
		}
...
//		layer_open(this.popupElement.attr("id"));			// layer_open 함수 호출
		...
		function getRletDstcCdAjax() {
		  	...
		  	$.ajax({
		  		url : "/admin/securedloan/srchCodeAjax.do",
		  		data : {
	...
	function srchRegisterAjax() {
		...
		$.ajax({
	  		url : "/admin/securedloan/srchRegisterAjax.do",
	  		data : {
	  			'custNo' : $('#custNo').val(), 							//고객번호
                'lonCnslNo' : $('#lonCnslNo').val(), 					//여신상담번호
	  		},
	  			...
	  			if(data.RSPCD == "0000") {
					...
					$('#rlrgstResult2 tbody').empty();
		       		...
		       		if((list_bsicGrid == null ||  list_bsicGrid.length == 0 )) {
		       			$('#rlrgstResult2 tbody').html('<tr><td colspan="10"><span>정보가 없습니다.</span></td></tr>');
					}
...
//				}
			...
			$.ajax({
				dataType : "json",
				type : "post",
				url : "/admin/securedloan/srchResultDetailAjax.do",
				data : {
					'custNo' : $('#custNo').val(), 				// 고객번호
	                'lonCnslNo' : $('#lonCnslNo').val(), 		// 여신상담번호
...
//		  					$('<option />', {value : item.cd, text : item.nm}).appendTo('.ccwCd');
		...
		function reqRegisterAjax() {
			...
			$.ajax({
		  		url : "/admin/securedloan/reqRegisterAjax.do",
		  		data : {
		  			'custNo' : $('#custNo').val(), 							//고객번호
	                'lonCnslNo' : $('#lonCnslNo').val(), 					//여신상담번호
		  			'inqDtbrCd' : "",										//조회부팀점코드공백
		...
		function registerPDFPrintAjax() {
			...
			*/
			...
			$.ajax({
		  		url : "/admin/securedloan/registerPDFPrintAjax.do",
		  		data : {
		...
		function srchRegisterDetailAjax() {
			...
			srchLandPriceLayerPopVo.issPropNo = item.issPropNo;
			$.ajax({
		  		url : "/admin/securedloan/srchRegisterDetailAjax.do",
		  		data : {
		...
		function getRletRdInfoAjax() {
			$.ajax({
				dataType : "json",
				type : "post",
				url : "/admin/securedloan/getRletRdInfoAjax.do",
				data : {
	                'custNo' : $('#custNo').val(), //고객번호
	                'lonCnslNo' : $('#lonCnslNo').val(), //여신상담번호
	                'rletDstc' : srchLandPriceLayerPopVo.rletDstc, //부동산구분
...
//					          	template	+= '<td><input type="radio" id="rRdPriceInfo{0}" name="rRdPriceInfo" title="선택"></td>';
		...
		function getPropertyPriceAjax() {
			$.ajax({
				dataType : "json",
				type : "post",
				url : "/admin/securedloan/getPropertyPriceAjax.do",
				data : {
					'custNo' : $('#custNo').val(), //고객번호
	                'lonCnslNo' : $('#lonCnslNo').val(), //여신상담번호
	                'rletUnqNo' : srchLandPriceLayerPopVo.rletUnqNo, //부동산고유번호
		...
		function getPropertyPriceAjax2() {
			$.ajax({
				dataType : "json",
				type : "post",
				url : "/admin/securedloan/getPropertyPriceAjax2.do",
				data : {
		...
		function getRadarPriceAjax() {
			$.ajax({
				dataType : "json",
				type : "post",
				url : "/admin/securedloan/getRadarPriceAjax.do",
				data : {
					'custNo' : $('#custNo').val(), //고객번호
	                'lonCnslNo' : $('#lonCnslNo').val(), //여신상담번호
