...
(function($) {
              ...
              , f = {
                ...
                redesign: function() {
                    ...
                    if (n.changeInput) {
                        ...
                        case 'boolean':
                            o = $('<div class="jFiler-input">' + '<div class="jFiler-input-caption"><span>' + f._assets.textParse(n.captions.feedback) + '</span></div>' + '<button type="button" class="jFiler-input-button"><span>' + f._assets.textParse(n.captions.button) + '</span></button>' + '</div>');
                            break;
                        case 'string':
                            if (n.changeInput != ' ')
                ...
                onEvent: function(e) {
                    switch (e.type) {
                    case 'focus':
                        p ? p.addClass('focused') : null;
                        break;
                    case 'blur':
                        p ? p.removeClass('focused') : null;
                        break;
                    case 'change':
                ...
                set: function(type, value) {
                    ...
                    case 'attrOpts':
                        ...
                        if (n.listInput === true) {
                            n.listInput = $('<input type="hidden" name="jfiler-list-' + s.attr('name').replace('[]', '').split('[').pop().replace(']', '') + '">').insertBefore(s)
                        }
                    ...
                    case 'disabled':
                        f.disabled = value;
                        p[f.disabled ? 'addClass' : 'removeClass']('jFiler-disabled');
                    ...
                    case 'feedback':
                        ...
                        $(!o.is(':file')) ? o.find('.jFiler-input-caption span').html(value) : null;
                        break;
                    case 'input':
                ...
                thumbnails: {
                    ...
                    item: function(item, replaceHtml) {
                        ...
                        item.image = '<div class="jFiler-item-image"></div>';
                        item.progressBar = f.isUploadMode() ? '<div class="jFiler-progressbar"><div class="bar"></div></div>' : '';
                        item.html = $(f._assets.textParse(item.appended && n.thumbnails.item2 ? n.thumbnails.item2 : n.thumbnails.item, item));
                        item.progressBar = item.html.find('.jFiler-progressbar');
                    ...
                    generateFileIcon: function(format, extension) {
                        var el = '<div style="${style}" class="jFiler-item-icon' + '${class}"><i>' + (extension ? extension : '') + '</i></div>';
                    ...
                    renderThumbnail: function(item, forceRender, src) {
                        var m = item.html.find('.jFiler-item-image')
                          , readerSkip = item.data && item.data.readerSkip
                          , setImageThumb = function(img) {
                            var $img = $(img);
                            m.removeClass('jFiler-no-thumbnail jFiler-loading').html($img);
                          ...
                          , setIconThumb = function() {
                            m.addClass('jFiler-no-thumbnail');
                            m.removeClass('jFiler-loading').html(item.icon);
                            if (item.html.hasClass('file-will-popup'))
                                item.html.removeClass('file-will-popup').addClass('file-has-popup');
                            n.thumbnails.onImageLoaded != null && $.isFunction(n.thumbnails.onImageLoaded) ? n.thumbnails.onImageLoaded(item, l, p, o, s) : null
                        }
                        ...
                        item.image = m.html('').addClass('jFiler-loading');
                    ...
                    popup: function(item, isByActions) {
                        ...
                        var container = $(n.thumbnails.popup.container)
                          , box = container.find('.jFiler-popup')
                          , hasArrowsClass = 'jFiler-popup-has-arrows'
                          , renderPopup = function() {
                            ...
                            if (item.reader.node) {
                                ...
                                if (popupIsNew)
                                    template.find('.jFiler-popup-node').html('<div class="reader-node"><div class="jFiler-popup-file-icon">' + item.icon + '</div></div>')
                            }
                        ...
                        if (box.length == 0)
                            box = $('<div class="jFiler-popup"></div>').appendTo(container);
                ...
                editor: {
                    rotate: function(item, degrees, force) {
                        ...
                        if (!inPopup) {
                            ...
                            var $popup = item.popup.html
                              , $node = $popup.find('.jFiler-popup-node')
                    ...
                    crop: function(item, data) {
                        ...
                        if (!inPopup) {
                            ...
                            if (!item.popup.editor.cropper) {
                                var template = '<div class="jFiler-cropper">' + '<div class="jFiler-cropper-area">' + '<div class="point point-a"></div>' + '<div class="point point-b"></div>' + '<div class="point point-c"></div>' + '<div class="point point-d"></div>' + '<div class="point point-e"></div>' + '<div class="point point-f"></div>' + '<div class="point point-g"></div>' + '<div class="point point-h"></div>' + '<div class="area-move"></div>' + '<div class="area-image"></div>' + '<div class="area-info"></div>' + '</div>' + '</div>'
                                  , $popup = item.popup.html
                                  , $imageEl = $popup.find('.jFiler-popup-node .reader-node > img')
                                  , $template = $(template)
                                  , $editor = $template.find('.jFiler-cropper-area');
                    ...
                    zoomer: function(item) {
                        ...
                        if (!item.popup.zoomer) {
                            ...
                            item.popup.zoomer = {
                                html: $popup.find('.jFiler-popup-zoomer'),
                                ...
                                mousedown: function(e) {
                                    ...
                                    if (zoomer.scale == 100 || zoomer.zoom == zoomer.scale)
                                        return;
                                    if (!zoomer.hasSpacePressed && $target[0] != $imageEl[0] && !$target.is('.jFiler-cropper'))
                                        return;
                                    if (eventType == 'mousedown') {
                                        callback()
                                    }
                        ...
                        var eventType = e.originalEvent.touches && e.originalEvent.touches[0] ? 'touchstart' : 'mousedown'
                          ...
                          , callback = function() {
                            f.sorter.sort = {
                                ...
                                placeholder: n.sorter.placeholder ? $(n.sorter.placeholder) : item.html.clone().addClass('jFiler-sorter-placeholder').html('')
                            };
                            $('body').css({
                                '-webkit-user-select': 'none',
                                '-moz-user-select': 'none',
                ...
                upload: {
                    ...
                    send: function(item, force_send) {
                        ...
                        p.addClass('jFiler-is-uploading');
                        if (item.upload.$ajax)
                        ...
                        item.upload.complete = function(jqXHR, textStatus) {
                            ...
                            if (g) {
                                p.removeClass('jFiler-is-uploading');
                        ...
                        item.upload.success = function(data, textStatus, jqXHR) {
                            if (item.upload.chunk && !item.upload.chunk.isLast) {
                                try {
                                    ...
                                    item.upload.chunk.temp_name = json.filer.temp_name
                                } catch (e) {}
                                return
                            }
                            delete item.upload.chunk;
                ...
                dragDrop: {
                    onDragEnter: function(e) {
                        ...
                        n.dragDrop.container.addClass('jFiler-dragging');
                    ...
                    onDragLeave: function(e) {
                        ...
                        f.dragDrop._timer = setTimeout(function(e) {
                            ...
                            n.dragDrop.container.removeClass('jFiler-dragging');
                            f.set('feedback', null);
                ...
                _assets: {
                    ...
                    copyAllAttributes: function(newEl, oldEl) {
                        ...
                        if (oldEl.get(0).filer)
                            newEl.get(0).filer = oldEl.get(0).filer;
                        return newEl
                    },
                    ...
                    textParse: function(text, opts, noOptions) {
                        ...
                        case 'string':
                            ...
                            text = text.replace(/\{\{fi-(.*?)\}\}/g, function(match, a) {
                                var a = a.replace(/ /g, '')
    ...
    $.filer = {
        getInstance: function(input) {
            ...
            return $input.length ? $input.get(0).filer : null
        }
    };
    $.fn.filer.defaults = {
        limit: null,
        ...
        theme: 'default',
        thumbnails: {
            box: '<div class="jFiler-items">' + '<ul class="jFiler-items-list"></ul>' + '</div>',
            boxAppendTo: null,
            item: '<li class="jFiler-item">' + '<div class="columns">' + '<div class="column-thumbnail">{{fi-image}}<span class="jFiler-action-popup"></span></div>' + '<div class="column-title">' + '<div title="{{fi-name}}">{{fi-name}}</div>' + '<span>{{fi-size2}}</span>' + '</div>' + '<div class="column-actions">' + '<button type="button" class="jFiler-action jFiler-action-remove" title="{{fi-captions.remove}}"><i class="jFiler-icon-remove"></i></a>' + '</div>' + '</div>' + '<div class="progress-bar2">{{fi-progressBar}}<span></span></div>' + '</li>',
            item2: '<li class="jFiler-item">' + '<div class="columns">' + '<div class="column-thumbnail">{{fi-image}}<span class="jFiler-action-popup"></span></div>' + '<div class="column-title">' + '<a href="{{fi-file}}" target="_blank">' + '<div title="{{fi-name}}">{{fi-name}}</div>' + '<span>{{fi-size2}}</span>' + '</a>' + '</div>' + '<div class="column-actions">' + '<a href="{{fi-file}}" class="jFiler-action jFiler-action-download" title="{{fi-captions.download}}" download><i class="jFiler-icon-download"></i></a>' + '<button type="button" class="jFiler-action jFiler-action-remove" title="{{fi-captions.remove}}"><i class="jFiler-icon-remove"></i></a>' + '</div>' + '</div>' + '</li>',
            popup: {
                container: 'body',
                loop: true,
                arrows: true,
                ...
                template: function(data) {
                    return '<div class="jFiler-popup-preview">' + '<button type="button" class="jFiler-popup-move" data-action="prev"><i class="jFiler-icon-arrow-left"></i></button>' + '<div class="jFiler-popup-node ${format}">' + '${reader.node}' + '</div>' + '<div class="jFiler-popup-content">' + '<div class="jFiler-popup-footer">' + '<ul class="jFiler-popup-tools">' + (data.format == 'image' && data.reader.node && data.editor ? (data.editor.cropper ? '<li>' + '<button type="button" data-action="crop">' + '<i class="jFiler-icon-crop"></i> ${captions.crop}' + '</button>' + '</li>' : '') + (data.editor.rotate ? '<li>' + '<button type="button" data-action="rotate-cw">' + '<i class="jFiler-icon-rotate"></i> ${captions.rotate}' + '</button>' + '</li>' : '') : '') + (data.format == 'image' ? '<li class="jFiler-popup-zoomer">' + '<button type="button" data-action="zoom-out">&minus;</button>' + '<input type="range" min="0" max="100">' + '<button type="button" data-action="zoom-in">&plus;</button>' + '<span></span> ' + '</li>' : '') + (data.data.url ? '<li>' + '<a href="' + data.file + '" data-action target="_blank">' + '<i class="jFiler-icon-external"></i> ${captions.open}' + '</a>' + '</li>' : '') + '<li>' + '<button type="button" data-action="remove">' + '<i class="jFiler-icon-trash"></i> ${captions.remove}' + '</button>' + '</li>' + '</ul>' + '</div>' + '<div class="jFiler-popup-header">' + '<ul class="jFiler-popup-meta">' + '<li>' + '<span>${captions.name}:</span>' + '<h5>${name}</h5>' + '</li>' + '<li>' + '<span>${captions.type}:</span>' + '<h5>${extension.toUpperCase()}</h5>' + '</li>' + '<li>' + '<span>${captions.size}:</span>' + '<h5>${size2}</h5>' + '</li>' + (data.reader && data.reader.width ? '<li>' + '<span>${captions.dimensions}:</span>' + '<h5>${reader.width}x${reader.height}px</h5>' + '</li>' : '') + (data.reader && data.reader.duration ? '<li>' + '<span>${captions.duration}:</span>' + '<h5>${reader.duration2}</h5>' + '</li>' : '') + '</ul>' + '<div class="jFiler-popup-info"></div>' + '<ul class="jFiler-popup-buttons">' + '<li><button type="button" class="jFiler-popup-button" data-action="cancel">${captions.cancel}</a></li>' + (data.editor ? '<li><button type="button" class="jFiler-popup-button button-success" data-action="save">${captions.confirm}</button></li>' : '') + '</ul>' + '</div>' + '</div>' + '<button type="button" class="jFiler-popup-move" data-action="next"><i class="jFiler-icon-arrow-right"></i></button>' + '</div>'
                },
            ...
            _selectors: {
                ...
                item: '.jFiler-item',
                start: '.jFiler-action-start',
                retry: '.jFiler-action-retry',
                remove: '.jFiler-action-remove',
                sorter: '.jFiler-action-sort',
                rotate: '.jFiler-action-rotate',
                popup: '.jFiler-popup-preview',
                popup_open: '.jFiler-action-popup'
            },
            beforeShow: null,
            onItemShow: null,
        ...
        captions: {
            ...
            paste: '<div class="jFiler-pending-loader"></div> Pasting a file, click here to cancel.',
            removeConfirmation: 'Are you sure you want to remove this file?',
            errors: {
                filesLimit: 'Only {{fi-limit}} files are allowed to be uploaded.',
                filesType: 'Only {{fi-extensions}} files are allowed to be uploaded.',
                fileSize: '{{fi-name}} is too large! Please choose a file up to {{fi-fileMaxSize}} MB.',
                filesSizeAll: 'Files that you chose are too large! Please upload files up to {{fi-maxSize}} MB.',
                fileName: 'File with the name {{fi-name}} is already selected.',
