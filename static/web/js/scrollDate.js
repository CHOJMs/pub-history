		...
		$.get(url, function(result) {
			...
			$layer.find(".date-box").eq(1).find("ul li").click(function(){
		    	...
		    	for(var i = 1; i <= lastDay; i++){
		    		if(i < 10){
		    			html +=	'<li style="cursor: pointer;"><a href="javascript:;" role="button" name="0'+i+'">0'+i+'일</a></li>';
		    		}else{
		    			html +=	'<li style="cursor: pointer;"><a href="javascript:;" role="button" name="'+i+'">'+i+'일</a></li>';
		    		}
		    	}
...
//		    	$("#dtHour").val($(this).children('a').text());
...
//		    	$("#dtMinute").val($(this).children('a').text());
		    ...
		    $layer.find("#dtBtn").click(function(){
		    	...
		    	if ($("#dtHour").val() == "") {
		    		$("#dtHour").val($("#dtHour").text().slice(0,-1));
		    	}

		    	if ($("#dtMinute").val() == "") {
		    		$("#dtMinute").val($("#dtMinute").text().slice(0,-1));
		    	}
		    ...
		    if(GlobalJSConfig.isApplyWA){
		        const selectBox = $(".date-box > a:first-of-type");

		        if(selectBox.length){
		            const selet = selectBox.next().find("li > a");
		            selectBox.next().attr("style","display:none;");
...
//		                }else
		                ...
		                if(!$(this).hasClass("open")){
		                    ...
		                    const selList = $(this).next().find("li > a");

		                    for(var el of selList){
		                    ...
		                    const handleClick = () => ($(this).removeClass("open"), $(this).next("ul").slideUp(), removeEventListener("click", handleClick));
		            ...
		            selet.click(function(){
		                ...
		                const selectList = $(this).closest("ul");
		                const selectBox = $(this).closest("ul").prev();

		                selectList.slideUp();
		                selectBox.removeClass("open");
		                selectBox.val(selectName);
		                selectBox.text(selectText);
