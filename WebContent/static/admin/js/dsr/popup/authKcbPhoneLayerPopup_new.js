...
//		layer_open(targetID);
		...
		*/
		...
		$layer.find(".popup-close").find('a').click(function() {
			uiCommon.closePopup("authKcbPhoneLayerPopup"); //예진
			// 레이어 요소 제거
		});
...
function phoneAuthClose(reqVo){
	...
	if(isRunning){
		console.log("타이머 중지 시키기");
		clearInterval(timer);
		console.log("휴대폰인증 팝업닫기2");
		$("#authTime").empty();
	}
...
function phoneAuthCodeCheck(){
    ...
    if(otpNo == "" || otpNo.length < 6) {
        alert("인증번호 요청 후 인증번호 6자리를 입력해주세요.");
        $("#OTP_NO").focus();
...
function openAuthKcbPhoneAgreeLayerPopup(agreeNo) {
	...
	var telComCd = $("#selectAuthLayerPop #mcpCd").val();
	...
	if (agreeNo == "1") {
		...
		var url1 = "https://safe.ok-name.co.kr/eterms/" + agreePath1 + ".jsp"; 		// 개인정보 수집/이용 동의 (통신사별)
		var url2 = "https://safe.ok-name.co.kr/eterms/" + agreePath2 + ".jsp";		// 고유 식별 정보 처리 동의 (공통)
		var url3 = "https://safe.ok-name.co.kr/eterms/" + agreePath3 + ".jsp";		// 서비스 이용 약관 (공통)
		var url4 = "https://safe.ok-name.co.kr/eterms/" + agreePath4 + ".jsp";		// 통신사 이용 약관 (통신사별)
...
//				data.birthday = TOUCHENEX_UTIL.Base64.decode(data.birthday);
				...
				authKcbPhoneErrInfo = "2005";
			} else {
//				authKcbPhoneErrInfo = data.RSPCD;
