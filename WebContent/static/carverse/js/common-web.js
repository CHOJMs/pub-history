...
var _MAIN_URL_ = "/carverse/main.do";		// 서비스 메인(MAIN) URL 지정, 특정 서비스의 메인으로 이동해야하는 경우 해당 서비스에서 해당 메인 정보 재 설정
//var _PLATFORM_ = "1";
...
jQuery(document).ready(function() {
	...
	// 페이지에서 해당 아이디를 가진 경우
	// 헤더, 푸터 숨김 처리
	if( $('#openWindowClose').length > 0 ) {
		// header, footer 숨김
		$("header, footer").hide();

//		if ( GlobalJSConfig.isMobileDevice ) {
//			$('#openWindowClose').show();
//		}
	}
 ...
 */
 function fn_mypageLogin(params) {
	// 로딩바 노출
	showLoading();
	...
	setTimeout(function() {
		$.ajax({
			url: "/carverse/mypage/mypageLogin.do",
			type: "post",
			dataType: "json",
			async: false,
			data: params,
			success: function (data) {
				...
				if(data.resultCode == "0000") {
					...
					var url = "/carverse/mypage/authStep.do";
...
//	closeGnb();  퍼블 나오면 헤더 Gnb 닫기 부분 수정할것......................................

	if (!$util.isEmpty(url)) {
		if ($util.isEmpty(linkTarget)) {
			// 로딩바
			if(isMobile.IOS()) {
				showLoading(true);
			} else {
...
function goMain () {
	...
	var url = _MAIN_URL_ || "/carverse/main.do";
