...
var SCROLLBAR_PADDING = 16; //PDF 양옆 margin 조절..
...
var PDFViewerApplication = {
  ...
  beforePrint: function beforePrint() {
    ...
    // S : 2018 홈페이지 고도화팀 : IE일경우 프린트 시도 시 다운로드 유도 로직 추가
    	...
    	if(navigator.userAgent.indexOf("MSIE") >= 0  || navigator.userAgent.indexOf("Trident") >= 0) // Trident for IE 11
   		{
    		...
    		alert("Internet Explorer 브라우저인 경우,\n서류 다운로드 기능만 제공됩니다.\n문서를 다운로드 받으신 후, PDF 뷰어를 통해 출력해 주세요.\n\n확인 버튼을 누르시면 문서파일이 자동으로 다운로드 됩니다.");
    		webViewerDownload(); // 문서 다운로드 요청
    		return;
   		}
   	}
    // E : 2018 홈페이지 고도화팀 : IE일경우 문서 다운로드로 유도 로직 추가
...
var PDFPageView = function () {
  function PDFPageView(options) {
    ...
    div.setAttribute('data-page-number', this.id);
    //마지막 div page 하단 공백 추가
    if(this.id == options.annotationLayerFactory.linkService.pdfDocument.pdfInfo.numPages && 1 != options.annotationLayerFactory.linkService.pdfDocument.pdfInfo.numPages ){
...
//          this.l10n.get('of_pages', { pagesCount: pagesCount }, 'of {{pagesCount}}').then(function (msg) {
          this.l10n.get('', { pagesCount: pagesCount }, '{{pagesCount}}').then(function (msg) {
